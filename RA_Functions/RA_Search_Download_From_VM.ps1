    <#  
      .SYNOPSIS
      Function to search for a file in a vm and restore to a user specified destination
      .DESCRIPTION
      Function to search for a file in a vm and restore to a user specified destination
      .NOTES
      Written by Jeremy Cathey for community usage
      .EXAMPLE  RA_Search_Download_From_VM -URI $ConnectionInfo.server -Creds $ConnectionInfo.creds
    #>
function RA_Search_Download_From_VM{
    param([parameter(Mandatory=$true)]$URI, [parameter(Mandatory=$true)]$Creds, [parameter(Mandatory=$false)]$VM_Name, [parameter(Mandatory=$false)]$File2Download, [parameter(Mandatory=$false)]$DestPath, [parameter(Mandatory=$false)]$OpenPath)
    ...
    $Address = 'https://'+$URI
    #Test if VM_Name is provided and if not, gather and test for actual presence in environment
    if($VM_Name -eq $null){$VM_Name = Read-Host -Prompt 'Enter name of the VM you wish to search: '}
        #Gather ID of Fileset
        $VM = Get-RubrikVM -Name $VM_Name
        If($VM -eq $null){Write-Host 'VM not found.  Aborting...'
                          Write-Host ""
                          return}
        $VMID = $VM.id
