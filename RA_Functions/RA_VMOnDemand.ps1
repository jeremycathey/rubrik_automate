...
#>
    ...
    param([parameter(Mandatory=$true)]$URI, [parameter(Mandatory=$true)]$Creds, [parameter(Mandatory=$true)]$LogPath, [parameter(Mandatory=$false)]$VMName)
    $auth = RA_GetAPIAuth -Creds $Creds
    If($VMName -eq $null){$VMName = Read-Host -Prompt 'Enter name of VM for on-demand snapshot: '}
